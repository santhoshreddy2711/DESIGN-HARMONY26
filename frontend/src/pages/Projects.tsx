...
const Projects: React.FC = () => {
  ...
  const loadHelpers = async () => {
    try {
      ...
      // Load designers
      const res = await fetch(`${API_BASE_URL}/api/reports/analytics?type=customer`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('dh_token')}` }
      });
      // In json db we can fetch from a generic users list
      const allUsersRes = await fetch(`${API_BASE_URL}/api/customers`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('dh_token')}` }
      });
  ...
  const handleUploadDesign = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/projects/${selectedProject.id}/upload-design`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('dh_token')}` },
        body: data
      });
  ...
  const handleUploadDoc = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/projects/${selectedProject.id}/upload-document`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('dh_token')}` },
        body: data
      });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {projects.map(proj => (
            <div 
              ...
              className={`p-5 bg-white dark:bg-darkcard border rounded-2xl cursor-pointer hover:shadow-md transition duration-200 flex flex-col justify-between h-48 ${
            ...
            >
              <div className="space-y-2">
                <div className="flex justify-between items-start">
                  <h3 className="font-bold text-sm dark:text-white truncate max-w-[170px]">{proj.project_name}</h3>
                  <span className="px-2.5 py-0.5 rounded-full font-semibold text-[9px] bg-gold-50 dark:bg-gold-950/20 text-gold-700 dark:text-gold-300">
                    {proj.status}
                  </span>
                </div>
              ...
              <div className="space-y-1.5 pt-3 border-t border-slate-50 dark:border-darkborder/50">
